...
class ReportCard(models.Model):
    ...
    # Official signature snapshots. Live staff signatures are used for draft
    # exports; once a card is finalized these snapshots preserve the exact
    # signatures and names that were approved for that academic record.
    teacher_signature_snapshot = models.ImageField(
        upload_to='report_card_signatures/%Y/%m/',
        null=True,
        blank=True,
    )
    teacher_signature_name = models.CharField(max_length=255, blank=True, default='')
    headteacher_signature_snapshot = models.ImageField(
        upload_to='report_card_signatures/%Y/%m/',
        null=True,
        blank=True,
    )
    headteacher_signature_name = models.CharField(max_length=255, blank=True, default='')
