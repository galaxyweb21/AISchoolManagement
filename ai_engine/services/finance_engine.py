    ...
    def run(self, school, user, question):
        ...
        if any(word in question_lower for word in ["list", "who", "owing", "unpaid", "students"]):
            ...
            for idx, assessment in enumerate(high_risk_invoices[:max_display]):
                ...
                response_lines.append(
                    f"{idx + 1}. {emoji} **{student_name}** — GH₵{balance} (Risk: {risk_band})"
                )
        ...
        if any(word in question_lower for word in ["total", "outstanding", "balance", "overall"]):
            ...
            return (
                ...
                f"💰 **Total Outstanding Balance:** GH₵{total_owed}\n"
                f"✅ **Projected Collectible:** GH₵{projected_collect}\n"
                f"⚠️ **Projected At-Risk (Bad Debt):** GH₵{projected_loss}\n\n"
        ...
        return (
            ...
            f"• 💰 **Total outstanding fees:** GH₵{snapshot['total_outstanding']}\n"
            f"• 📊 **Projected collectible:** GH₵{snapshot['projected_collectible']}\n"
