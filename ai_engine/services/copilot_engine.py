...
This school operates in Ghana. Every monetary amount you mention -
fees, balances, payroll, budgets, or any other figure - is in Ghana
Cedis. Always format money as "GH₵" followed by the amount (e.g.
"GH₵1,200.00"). Never use "$", "USD", or any other currency symbol.
...
"""
    ...
    def answer(
        ...
        # --------------------------------------------------------------
        # If the AI call produced nothing, surface *why* to admins.
        #
        # AIService.LAST_ERROR is set by AIService._call_groq() on every
        # failure path (missing key, bad key, rate limit, network error,
        # etc). Without this, every failure looked identical to the user
        # ("I could not generate a complete answer") whether the real
        # cause was a missing GROQ_API_KEY (a one-time deployment/config
        # issue an admin needs to fix in the Render dashboard) or a
        # transient Groq outage (worth just retrying). Only school/
        # platform admins see the underlying reason - other roles keep
        # the plain retry message so this never reads as a stack trace
        # to a parent or teacher.
        # --------------------------------------------------------------

        admin_diagnostic = ""

        if not answer and AIService.LAST_ERROR:
            ...
            if self.policy.get("label") in admin_labels:

                admin_diagnostic = (
                    "AI service is not configured: no GROQ_API_KEY is "
                    "set for this deployment. Add it in the Render "
                    "dashboard under this service's Environment "
                    "variables, then redeploy."
                    if "GROQ_API_KEY is not configured" in AIService.LAST_ERROR
                    else (
                        "AI service error: "
                        + AIService.LAST_ERROR[:300]
                    )
                )
