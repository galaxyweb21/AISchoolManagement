    ...
    def _call_groq(system_prompt, user_prompt, max_tokens=200, temperature=0.7, model=None):
        ...
        for candidate in AIService._model_candidates(model):
            payload = {
                'model': candidate,
                'messages': messages,
                # Groq's API reference marks "max_tokens" as deprecated in
                # favor of "max_completion_tokens" (same meaning, current
                # name) - using the current name here avoids relying on
                # deprecated-parameter handling for every single Copilot/
                # report-comment/narrative call this service makes.
                'max_completion_tokens': max_tokens,
                'temperature': temperature,
                'include_reasoning': False,
                'reasoning_effort': 'low',
