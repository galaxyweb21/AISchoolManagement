...
class SafePasswordResetView(auth_views.PasswordResetView):
    ...
    SMTP failures must not turn the password-reset page into a 500 or leave
    a Gunicorn worker hanging indefinitely.  The actual email configuration
    remains environment-driven in settings.py.
    """
    ...
    def form_valid(self, form):
        ...
        try:
            # Django's PasswordResetForm intentionally returns the same success
            # response whether an account exists or not. Log the delivery
            # attempt (never the password/token) so SMTP failures on Render
            # are diagnosable without exposing credentials.
            ...
            form.add_error(
                ...
                "Please try again shortly or contact the school administrator."
            )
            return self.form_invalid(form)
...
def profile_view(request):
    ...
    if request.method == 'POST':
        ...
        if errors:
            ...
            # Keep staff password-change tracking in sync with the actual
            # Django password hash update.
            if new_password and getattr(user, 'role', '') in [
                'SCHOOL_ADMIN', 'BURSAR', 'REGISTRAR', 'HOD',
                'SECRETARY', 'TEACHER'
            ]:
                try:
