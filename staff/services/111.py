        ...
        return JsonResponse(
            {
                ...
                "error": (
                    ...
                    "and staff position are required."
                ),
            },
            status=400,
        )
...
def staff_delete(request, staff_id):
    ...
    staff.delete()
    return JsonResponse({'success': True, 'message': f"Staff '{staff.user.get_full_name()}' deleted successfully."})
...
def leave_request_create(request):
    ...
    if requested_staff_id and not can_delegate:
        return JsonResponse({"success": False, "error": "You cannot create leave for another staff member."},
                            status=403)
    ...
    if not all([leave_type_id, start_raw, end_raw, reason]):
        return JsonResponse({"success": False, "error": "Leave type, start date, end date and reason are required."},
                            status=400)
    ...
    if end_date < start_date:
        return JsonResponse({"success": False, "error": "The end date cannot be before the start date."}, status=400)
    if start_date.year != end_date.year:
        return JsonResponse({"success": False, "error": "A leave request cannot currently cross two calendar years."},
                            status=400)
    ...
    if leave_type.requires_documentation and not supporting_document:
        return JsonResponse({"success": False, "error": f"Supporting documentation is required for {leave_type.name}."},
                            status=400)
