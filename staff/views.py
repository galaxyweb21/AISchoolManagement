...
def staff_create(request):
    ...
    is_headteacher = request.POST.get("is_headteacher") == "1"
    signature_is_active = request.POST.get("signature_is_active") == "1"
    ...
    try:
        ...
        with transaction.atomic():
            ...
            staff = StaffProfile.objects.create(
                ...
                signature_uploaded_at=timezone.now() if signature_image else None,
                signature_is_active=bool(signature_image) and signature_is_active,
            ...
            if is_headteacher:
                StaffProfile.objects.filter(
                    school=school, is_headteacher=True
                ).exclude(id=staff.id).update(is_headteacher=False)
    ...
    try:
        ...
        with transaction.atomic():
            ...
            if remove_signature:
                ...
                staff.signature_uploaded_at = None
                staff.signature_is_active = False
            elif signature_image:
                staff.signature_image = signature_image
                staff.signature_uploaded_at = timezone.now()
                staff.signature_is_active = signature_is_active
            else:
                staff.signature_is_active = bool(staff.signature_image) and signature_is_active

            staff.is_headteacher = is_headteacher
            if is_headteacher:
                StaffProfile.objects.filter(
                    school=school, is_headteacher=True
                ).exclude(id=staff.id).update(is_headteacher=False)
