...
class TenantQuerySet(models.QuerySet):
    ...
    def delete(self):
        ...
        if active_school:
            # IMPORTANT: calling ``filtered.delete()`` here would dispatch back
            # into this overridden method and recurse forever.  Call Django's
            # base QuerySet.delete() explicitly so the tenant filter is applied
            # exactly once.
            filtered = self.filter(**{self.school_field: active_school})
            return models.QuerySet.delete(filtered)
