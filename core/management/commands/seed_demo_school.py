...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            # Give the demo administrator a proper HR staff profile so the
            # report-card signature feature can be demonstrated end-to-end.
            admin_profile, _ = StaffProfile.objects.get_or_create(
                school=school, user=admin,
                defaults={"staff_id": "DEMO-HEAD-001", "staff_position": "SCHOOL_ADMIN", "is_headteacher": True},
            )
            admin_profile.staff_position = "SCHOOL_ADMIN"
            admin_profile.is_headteacher = True
            admin_profile.default_password = password
            admin_profile.save(update_fields=["staff_position", "is_headteacher", "default_password"])
