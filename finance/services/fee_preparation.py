...
def prepare_student_fee(
    ...
    """
    Idempotently prepare one student's fee for one academic term.

    Enterprise billing rule:
      * one StudentFee per student + academic term;
      * PREPARED is a frozen calculation;
      * APPROVED/INVOICED records are immutable through normal preparation;
      * repeating Prepare/Approve never adds another charge;
      * a rebuild is only allowed before approval and never silently changes
        an approved/invoiced financial record.
    """
    if not student or not student.school:
        raise FeePreparationError("Student does not belong to a school.")
    if not academic_term or academic_term.academic_year.school_id != student.school_id:
        raise FeePreparationError("Academic term does not belong to the student's school.")

    # The term is the billing boundary. Never identify a fee only by
    # fee_structure: the same structure can be reused in different terms.
    existing = (
        StudentFee.objects
        .select_for_update()
        .filter(
    ...
    if existing:
        if existing.status == 'CANCELLED':
            raise FeePreparationError(
                f"Fee preparation for {student} has been cancelled."
            )
        if existing.status in ('APPROVED', 'INVOICED'):
            # Idempotency: an approved/invoiced charge is authoritative and
            # must never be rebuilt merely because Prepare is clicked again.
            return existing
        if existing.status == 'PREPARED' and not force_rebuild:
            # Prepared is a frozen snapshot. Re-running Prepare is a no-op.
            return existing
        student_fee = existing
        if force_rebuild:
            # Rebuild is a deliberate pre-approval operation only. Existing
            # manual adjustments remain attached and are reapplied below.
        ...
        fee_structure = package.fee_structure if package and package.fee_structure else get_student_fee_structure(student, academic_term)
        enrollment_type = package.enrollment_type if package else get_student_enrollment_type(student, academic_term)
    ...
    # Resolve the authoritative fee structure/enrollment for this term.
    package = get_enrollment_fee_package(student, academic_term)
    ...
    # Keep the snapshot tied to the structure actually used for this term.
    if student_fee.fee_structure_id != fee_structure.id:
    ...
    for structure_item in fee_structure.items.all():
        ...
        include_item = True
        if category.is_optional and category.category_type == 'TRANSPORT':
            try:
                include_item = bool(student.transport.is_active)
            except Exception:
                include_item = False
        if not include_item:
            continue
        ...
        StudentFeeItem.objects.create(
            student_fee=student_fee, fee_category=category,
            description=f"{category.name} - {academic_term.name}",
            standard_amount=standard_amount, discount_amount=discount,
            adjustment_amount=ZERO, final_amount=final_amount,
    ...
    for item in list(student_fee.items.order_by('created_at')):
    ...
    if include_addons:
        for addon in get_applicable_class_addons(student, academic_term):
            ...
            if not existing_addon:
                StudentFeeItem.objects.create(
                    student_fee=student_fee, fee_category=addon['fee_category'],
                    description=addon['description'], standard_amount=addon['amount'],
                    discount_amount=ZERO, adjustment_amount=ZERO,
                    final_amount=addon['amount'], is_optional=not addon['is_required'],
                    is_waived=False,
                )
            elif existing_addon.standard_amount != addon['amount']:
        ...
        for addon in get_applicable_legacy_addons(student, academic_term):
            existing_addon = student_fee.items.filter(
                fee_category=addon['fee_category'],
                description__icontains=addon['addon_name']
            ).first()
            if not existing_addon:
                StudentFeeItem.objects.create(
                    ...
                    description=addon['description'], standard_amount=addon['amount'],
                    discount_amount=ZERO, adjustment_amount=ZERO,
                    final_amount=addon['amount'], is_optional=True, is_waived=False,
                )

    total_adjustments = ZERO
    for adjustment in get_student_adjustments(student_fee):
        ...
        if not existing_adj:
            ...
            StudentFeeItem.objects.create(
                student_fee=student_fee, fee_category=adjustment.fee_category,
                description=adjustment.description, standard_amount=abs(adj_amount),
                discount_amount=ZERO if adj_amount > 0 else abs(adj_amount),
                adjustment_amount=adj_amount, final_amount=adj_amount if adj_amount > 0 else ZERO,
                is_optional=True, is_waived=adj_amount < 0,
            )
        if adjustment.adjustment_type != 'DISCOUNT':
            total_adjustments += adjustment.amount

    current_items_total = student_fee.items.aggregate(total=Sum('final_amount'))['total'] or ZERO
    ...
    student_fee.save(update_fields=[
        'fee_structure', 'base_amount', 'discount_amount', 'adjustment_amount',
        'arrears_amount', 'final_amount', 'status', 'prepared_by', 'updated_at'
    ])

    if enrollment_type:
        StudentFeeEnrollment.objects.update_or_create(
            school=student.school, student=student, academic_term=academic_term,
            defaults={
                'enrollment_type': enrollment_type, 'package_used': package,
                'is_automatic': True, 'applied_by': prepared_by,
            }
        )
...
def prepare_class_fees(
    ...
    for student in students:
        try:
            existing = (
                StudentFee.objects
                .filter(school=school, student=student, academic_term=academic_term)
                .first()
            )
            ...
            if existing and existing.status in ('PREPARED', 'APPROVED', 'INVOICED') and not force_rebuild:
                unchanged.append(student_fee)
            else:
...
def auto_prepare_student_fees(student, created_by=None, raise_errors=False):
    ...
    try:
        ...
        # If this class/term has already entered the approved billing cycle,
        # a late registration must follow that cycle even if the enrollment
        # type's generic auto-approve switch is off. This is important when a
        # school has already prepared/approved the class before registering a
        # new student.
        class_billing_started = StudentFee.objects.filter(
            school=student.school,
        ...
        # Respect the school's normal enrollment lifecycle settings. If
        # automatic preparation is disabled, only the already-approved class
        # lifecycle above may trigger automatic preparation.
        if not enrollment.enrollment_type.auto_prepare_fees and not class_billing_started:
            logger.info(
        ...
        student_fee = prepare_student_fee(
            ...
            # Automatic registration must be idempotent. Never rebuild an
            # existing PREPARED/APPROVED/INVOICED fee here.
            force_rebuild=False,
            include_addons=True,
        )

        if enrollment.enrollment_type.auto_approve_fees or class_billing_started:
