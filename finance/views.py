...
def api_prepare_class_fees(request):
    ...
    try:
        ...
        message_parts = []
        if result['prepared_count']:
            message_parts.append(f"Prepared {result['prepared_count']} new student fee(s).")
        if result.get('unchanged_count'):
            message_parts.append(f"{result['unchanged_count']} fee(s) were already prepared/approved and were left unchanged.")
        if result['error_count']:
            message_parts.append(f"{result['error_count']} student(s) could not be processed.")
        if not message_parts:
            message_parts.append('No fee changes were required.')
...
def api_student_fee_approve(request, student_fee_id):
    ...
    if student_fee.status == 'INVOICED':
        return JsonResponse({
            'success': True,
            'message': 'This student fee is already invoiced. No changes were made.',
            'status': student_fee.status,
            'idempotent': True,
        })
    ...
    try:
        ...
        for fee_id in fee_ids:
            try:
                ...
                if student_fee.status == 'INVOICED':
                    # Idempotent bulk approval: an invoiced fee is already in
                    # the official billing cycle, so it is not touched.
                    continue
